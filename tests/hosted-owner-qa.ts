/** Coordinated HOSTED QA. Real HTTPS gateway, one private journal per business; no fixtures. */
...
let journalPath = '';
type Phase = 'prepare' | 'create' | 'inspect' | 'conflict' | 'reschedule' | 'cancel' | 'isolation';
interface Journal {
  version: 1; profile: string; publicOrigin: string; ownerApiOrigin: string; label: string; site: Storefront; input: BookingInput;
  createKey: string; firstSlot: Slot; replacementSlot: Slot; createDispatched?: boolean;
  created?: BookingResult;
  unexpectedConflictResult?: BookingResult;
  conflictVerified?: boolean;
...
try {
  ...
  assert.equal(process.env.STOREFRONT_HOSTED_QA_APPROVED, '1');
  const profile = profileName(process.argv[2]);
  const phase = process.argv[3] as Phase;
  assert.ok(['prepare', 'create', 'inspect', 'conflict', 'reschedule', 'cancel', 'isolation'].includes(phase));
  stage = 'validated production binding';
  const operatorDirectory = await externalOperatorDirectory(defaultOperatorDirectory(), process.cwd());
  const deployment = validateProfile(await readOperator(operatorDirectory, profile), profile, 'production');
  assert.equal(deployment.org, 'lugearma');
  journalPath = join(operatorDirectory, `${profile}.hosted-qa.json`);
  ...
  function unpaid(site: Storefront) {
    ...
    assert.equal(!!site.capabilities.grooming, profile === 'best-in-show-grooming');
  }
  ...
  function report(phase: Phase, journal: Journal, receipt?: Receipt) {
    ...
    console.log(JSON.stringify({
      phase, profile, marker: journal.label, reference: receipt?.reference,
  ...
  if (phase === 'prepare') {
    ...
    assert.ok(process.argv[4], 'Provide the coordinator-selected service/date file');
    const selectionFile = resolve(process.argv[4]);
    await externalOperatorDirectory(selectionFile, process.cwd());
    const selectedStat = await lstat(selectionFile);
    assert.ok(selectedStat.isFile() && !selectedStat.isSymbolicLink() && selectedStat.size < 10000 && !(selectedStat.mode & 0o077));
    const selection = object(JSON.parse(await readFile(selectionFile, 'utf8')));
    assert.equal(selection.profile, profile);
    assert.equal(selection.publicOrigin, origin);
    assert.equal(selection.ownerApiOrigin, deployment.ownerApiOrigin);
    ...
    const label = `QA Hosted ${profile === 'fashion-style' ? 'Fashion' : 'Grooming'} ${randomUUID().slice(0, 8)}`;
    const input = parseBookingInput({
      ...
      pet: site.capabilities.grooming ? { name: `QA Mascota ${label.slice(-8)}`, sizeId: null, breed: null, ageMonths: null, behaviorNotes: '', groomingPreferences: '', photoUploadId: null } : null,
    });
    const journal: Journal = { version: 1, profile, publicOrigin: origin, ownerApiOrigin: deployment.ownerApiOrigin, label, site, input, createKey: randomUUID(), firstSlot, replacementSlot };
    ...
    assert.equal(journal.publicOrigin, origin);
    assert.equal(journal.ownerApiOrigin, deployment.ownerApiOrigin);
    assert.ok(journal.label.startsWith('QA Hosted '));
    assert.ok(!journal.unexpectedConflictResult, 'Unexpected conflict result needs coordinator recovery before further QA');
    ...
    if (phase === 'create') {
      ...
      unpaid(site);
      if (phase === 'conflict') {
        stage = 'same create key with changed payload must conflict without a write';
        try {
          const unexpected = await api.book(journal.site, { ...journal.input, notes: `${journal.input.notes} Conflict probe.` }, journal.createKey, journal.firstSlot);
          journal.unexpectedConflictResult = unexpected; await save(journal);
          assert.fail('Conflict accepted; result saved privately for coordinator recovery');
        } catch (error) {
          assert.ok(error instanceof StorefrontError && error.code === 'idempotency_conflict' && !error.uncertain);
        }
        journal.conflictVerified = true; await save(journal);
      } else if (phase === 'isolation') {
        stage = 'cross-business capabilities must be rejected by the other deployed gateway';
        const peer = profileName(process.argv[4]); assert.notEqual(peer, profile);
        const peerBinding = validateProfile(await readOperator(operatorDirectory, peer), peer, 'production');
        assert.notEqual(peerBinding.siteId, deployment.siteId); assert.notEqual(peerBinding.publicOrigin, origin);
        assert.notEqual(peerBinding.ownerApiOrigin, deployment.ownerApiOrigin);
        for (const [path, token] of [['booking-status', journal.created.receiptToken], ['guest-booking', journal.created.managementToken]]) {
          const response = await fetch(new URL(`/api/storefront/v1/${path}`, peerBinding.publicOrigin), { method: 'POST', redirect: 'error', signal: AbortSignal.timeout(15000), headers: { Origin: peerBinding.publicOrigin, 'Content-Type': 'application/json' }, body: JSON.stringify({ token }) });
          assert.equal(response.status, 404); assert.deepEqual(await response.json(), { code: 'link_invalid' });
        }
    ...
    assert.equal(Date.parse(guest.actions.deadlineAt), Date.parse(current.startsAt) - 24 * 60 * 60 * 1000);
    if (current.state === 'confirmed') {
      assert.ok(Date.now() < Date.parse(guest.actions.deadlineAt));
      assert.equal(guest.actions.canCancel, true); assert.equal(guest.actions.canReschedule, true);
    }
