...
test('Checkout handoff and cancelled return reuse the existing booking without automatic redirect loop', async ({ page, request }) => {
  ...
  await page.goto(`http://127.0.0.1:5375/reserva/pago?success=true#manage=${managementToken(site)}`);
  await expect(page.getByRole('heading', { name: 'Anticipo pendiente' })).toBeVisible();
  await expect(page).toHaveURL('http://127.0.0.1:5375/reserva/pago');
...
test('lost create reply replays the same command, reconciles paid state and skips stale Checkout', async ({ page, request }) => {
  ...
  await page.route('**/api/storefront/v1/booking-status', route => route.fulfill({ json: { ...receipt(site, 'confirmed'), paymentStatus: 'paid' } }));
  await fill(page); await page.getByRole('button', { name: 'Continuar al anticipo' }).click();
...
for (const [state, paymentStatus, heading] of [
  ['expired', 'unpaid', 'La reserva venció'], ['confirmed', 'paid', 'Cita confirmada'],
] as const) {
