...
export function SlotPicker({ site, serviceId, api, selected, onSelect, disabled = false, refresh = 0 }: {
  ...
  useEffect(() => {
    ...
    api.availability(site, serviceId, date, controller.signal).then(data => {
      if (!controller.signal.aborted) setResult({ date, slots: data.slots });
  ...
  return <fieldset className="booking-fields slot-picker" disabled={disabled}>
    <legend>Selecciona fecha y hora para tu cita</legend>
    <BookingCalendar date={date} min={site.booking.firstDate} max={site.booking.lastDate} locale={site.locale} today={dateInZone(new Date().toISOString(), site.branch.timeZone)} onChange={value => { onSelect(null); setDate(value); }} />
    <div className="slot-options" aria-label="Horario y profesional" role="group" aria-busy={loading}>
      <p className="slots-label">Horarios disponibles · {date}</p>
