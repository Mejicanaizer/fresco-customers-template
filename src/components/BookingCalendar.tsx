...
export function BookingCalendar({ date, min, max, locale, today, onChange }: { date: string; min: string; max: string; locale: string; today: string; onChange: (date: string) => void }) {
  ...
  const days = Array.from({ length: 42 }, (_, i) => shiftDate(first, i - offset));
  ...
  return <div className="booking-calendar" role="group" aria-label="Fecha de tu cita">
    ...
    <div className="calendar-days" role="group" aria-labelledby={`${id}-month`}>
      {days.map(value => <button id={`${id}-${value}`} key={value} type="button" data-date={value} className={`calendar-day ${value.slice(0, 7) !== month ? 'outside-month' : ''}`} aria-label={dayLabel(value)} aria-pressed={value === date} aria-current={value === today ? 'date' : undefined} disabled={value < min || value > max} tabIndex={value === focusDate ? 0 : -1} onKeyDown={e => keyboard(e, value)} onFocus={() => setFocusDate(value)} onClick={() => { setMonth(value.slice(0, 7)); setFocusDate(value); onChange(value); }}>{Number(value.slice(-2))}</button>)}
    </div>
  </div>;
}
